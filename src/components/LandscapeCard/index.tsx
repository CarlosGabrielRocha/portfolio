...
const LandscapeCard: React.FC<LandscapeProps> = ({ children, className, badges, imgSrc, imgAlt }) => {
  const styles = `flex lg:w-3xl gap-4 p-10 bg-black/30 shadow-sm shadow-black hover:bg-gradient-to-br hover:from-white/5 hover:to-hlight-blue/5 hover:outline hover:shadow-white/15 hover:shadow-md backdrop-blur-lg rounded-2xl ${className}`
  return (
    <div className={styles}>
        <LandscapeContent className="flex flex-col flex-2 gap-4">
          { children }
...
const LandscapeContent: React.FC<DefaultProps> = ({ children, className }) => {
  return (
    <div className={`flex flex-col flex-2 gap-4 ${className}`}>{children}</div>
  )
}
